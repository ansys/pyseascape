...
class SCApp(object):
    def __init__(self, url=None, executable=None, executable_work_dir='gp_rr', debug_remote=False):
        ...
        pp = subprocess.Popen([f'{executable}', '--message_converter', 'off', '--work_dir', executable_work_dir, '-c', f'explorer_sc.run_communicator({remote_args})'], stdout=subprocess.PIPE)
        self.rh_process_ = pp
    ...
    def send_cmd(self, cmd, has_return=True):
        try:
            d = requests.post(self.url_, data={'command':cmd, 'return':has_return, 'delayed':False})
    ...
    def terminate(self, local_only=False):
        ...
        except requests.exceptions.ConnectionError: # rhsc side needs to be fixed so that we don't get this exception
            pass
        self.url_ = None
...
class ObjConverter(object):
    ...
    def _convert_func(self, ss):
        for cls_name in ['View', 'SeaScapeDB', 'LayoutWindow', 'Launcher']: ## these are Remotable
...
def _inject_global_commands(rh):
    ...
    appsc_modules = ['package', 'bqm_current', 'deco'] #TODO: add more here for commmonly used appsc modules
    for cc in appsc_modules:
        f_globals[cc] = SCAppModule(cc, rh)


    rh_symbols = ['open_db', 'open_scheduler_window', 'create_grid_launcher', 'create_local_launcher', 'register_default_launcher', 'get_default_options']
